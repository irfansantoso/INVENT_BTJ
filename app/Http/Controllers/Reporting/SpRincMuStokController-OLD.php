...
use App\Exports\ExportSpRincMuStok;
...
class SpRincMuStokController extends Controller
{
    public function spRincMuStok(Request $request)
    {
        $bulan = Helper::bulan();
        $data['title'] = 'SP Rincian Mutasi Stok';
        ...
        return view('reporting/rpt_spRincMuStok', $data,compact('bulan'));
    }

    public function spRincMuStok_rpt(Request $request)
    {   
        // dd($request);
        ...
        $getSumNilai = DB::select(DB::raw("SELECT z.kel_brg, z.kd_brg, z.nm_brg as nmbrg, z.ukuran as ukuran, z.satuan as satuan,
                                           ...
                                           CASE WHEN SUM(z.pengeluaran2Qty) > 0 
                                                THEN ((SUM(z.saldoAwal)-SUM(z.pengeluaran1)) + SUM(z.penerimaan))-((((SUM(z.saldoAwal)-SUM(z.pengeluaran1)) + SUM(z.penerimaan)) / ((SUM(z.saQty)-SUM(z.pengeluaran1Qty)) + SUM(z.penerimaanQty))) * SUM(z.pengeluaran2Qty))
                                                ELSE 
                                    ...
                                    FROM (
                                        SELECT a.kd_brg AS kd_brg, a.nm_brg AS nm_brg, a.ukuran as ukuran, a.satuan as satuan, a.kel_brg, 
                                               CASE WHEN a.kode_periode < $gabYm AND a.from_table IN ('import','PindahGudang','Retur','KoreksiSO') THEN a.qty ELSE 0 END as saQty,
                                               ...
                                               CASE WHEN a.kode_periode = $gabYm AND a.from_table IN ('import','PindahGudang','Retur','KoreksiSO') THEN a.qty ELSE 0 END as penerimaanQty,
                                               ...
                                               CASE WHEN a.kode_periode < $gabYm AND a.from_table IN ('PemSpBbm','PemPinGudSpBbm','ReturPemSpBbm','KoreksiPemSpBbm') THEN a.qty ELSE 0 END as pengeluaran1Qty,
                                               ...
                                               CASE WHEN a.kode_periode = $gabYm AND a.from_table IN ('PemSpBbm','PemPinGudSpBbm','ReturPemSpBbm','KoreksiPemSpBbm') THEN a.qty ELSE 0 END as pengeluaran2Qty,
                                               CASE WHEN a.kode_periode <= $gabYm AND a.from_table IN ('PemSpBbm','PemPinGudSpBbm','ReturPemSpBbm','KoreksiPemSpBbm') THEN a.qty ELSE 0 END as pengeluaran3Qty,
                                               CASE WHEN a.kode_periode <= $gabYm AND a.from_table IN ('import','PindahGudang','Retur','KoreksiSO') THEN a.qty ELSE 0 END as saldoAkhirQty,
        ...
        $fileNm = "SP Rincian Mutasi Stok ".$bln."_".$thn.".xlsx";
        return Excel::download(new ExportSpRincMuStok($bln,$thn,$array), $fileNm);
