...
<!-- Content Header (Page header) -->
    ...
    <div class="card card-primary">
      ...
      <form class="form-horizontal" action="{{ route('trDetailKoreksiSo.add') }}" method="POST">
         @csrf        
        <input type="hidden" name="id_head_koreksi_so" id="id_head_koreksi_so" value="{{ $getHeaderKoreksiSo->id }}">
        <input type="hidden" name="no_ref" id="no_ref" value="{{ $getHeaderKoreksiSo->no_ref }}">
        <input type="hidden" name="tgl_det_koreksi_so" id="tgl_det_koreksi_so" value="{{ $getHeaderKoreksiSo->tgl_koreksi }}">
        <input type="hidden" name="no_koreksi" id="no_koreksi" value="{{ $getHeaderKoreksiSo->no_koreksi }}">
        <input type="hidden" name="supplier" id="supplier" value="{{ $getHeaderKoreksiSo->supplier }}">
        <input type="hidden" name="kode_periode" id="kode_periode" value="{{ $getHeaderKoreksiSo->kode_periode }}">
        <input type="hidden" name="kd_area" id="kd_area" value="{{ $getHeaderKoreksiSo->kd_area }}">
        ...
        <div class="card-body">          
            ...
            <div class="col-sm-2">
              <div class="form-group">
                <label>No Ref :</label><br/>
                  {{ $getHeaderKoreksiSo->no_ref }}
              </div>
            </div>
            <div class="col-sm-2">
              <div class="form-group">
                <label>No Retur :</label><br/>                
                  {{ $getHeaderKoreksiSo->no_koreksi }}
              </div>
            </div>
            <div class="col-sm-2">
              <div class="form-group">
                <label>Tgl :</label><br/>                
                  {{ $getHeaderKoreksiSo->tgl_koreksi }}
              </div>
            </div>         
          </div>
          <div class="row">
            <div class="col-sm-2">
              <div class="form-group">
                <label>Area :</label><br/>
                  {{ $getHeaderKoreksiSo->kd_area }}-{{ $getHeaderKoreksiSo->nmarea }}
              </div>
            </div>            
            <div class="col-sm-4">
              <div class="form-group">
                <label>Keterangan :</label><br/>                
                  {{ $getHeaderKoreksiSo->keterangan }} {{ $getHeaderKoreksiSo->kode_periode }}
              </div>
            </div>         
          </div>
          <hr>
          ...
          <div class="row">
            ...
            <div class="col-sm-2">
              <div class="form-group">
                <label>Tanggal</label>
                  <input type="text" class="form-control" id="tgl_det_koreksi_so" name="tgl_det_koreksi_so" value="{{ $getHeaderKoreksiSo->tgl_koreksi }}" readonly>
              </div>
            </div>
          </div>
        <!-- /.card-body -->
    ...
    <div class="card">
      <!-- /.card-header -->
      <div class="card-body">
        <table id="trDetailKoreksiSo" class="table table-bordered table-striped">
          <thead>
          <tr>
            <th>Kd Brg</th>
            <th>Nama Brg</th> 
          ...
          <tbody>
            @foreach ($getDetailKoreksiSo as $gds)
            <tr>              
                <td>{{ $gds->kdbrg }}</td>
                <td>{{ $gds->partnumb }}</td>
                <td>{{ $gds->ukuran }}</td>
                ...
                <td>{{ $gds->tgl_det_koreksi_so }}</td>
                <td>                
                  <a href="#" data-toggle="modal" 
                  data-target="#modalEdit" 
                  data-id="{{ $gds->id }}"
                  data-id_head_koreksi_so="{{ $gds->id_head_koreksi_so }}"
                  ...
                  data-kode_periode="{{ $getHeaderKoreksiSo->kode_periode }}"
                  data-tgl_koreksi="{{ $getHeaderKoreksiSo->tgl_koreksi }}" class="edit btn btn-primary btn-sm editDetKoreksiSo" title="Edit"><i class="far fa-edit"></i></a>
                  <a href="#" data-toggle="modal" data-target="#modal-delete" data-id="{{ $gds->id }}" class="btn btn-danger btn-sm delDetKoreksiSo" title="Delete"><i class="fa fa-trash"></i></a>
                </td>
            </tr>
            @endforeach                     
          </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width: 80%;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div id="editForm"></div>           
                    <form class="form-horizontal" action="{{ route('trDetailKoreksiSo.edit') }}" method="POST">
                       @csrf
                      <input type="hidden" class="form-control" id="idx" name="id">
                      <input type="hidden" name="id_head_koreksi_sox" id="id_head_koreksi_so">
                      ...
                      <div class="card-body">          
                        ...
                        <div class="row">
                          ...
                          <div class="col-sm-2">
                            <div class="form-group">
                              <label>Tanggal</label>
                                <input type="text" class="form-control" id="tglx" name="tgl_det_koreksi_so" readonly>
                            </div>
                          </div>
                        </div>
                      <!-- /.card-body -->
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="card">
                      <!-- /.card-header -->
                      <div class="card-body">
                        <table id="stInvKoreksiSo" class="table table-bordered table-striped responsive">
                          <thead>
                          <tr>
                            <th>Kd_Brg</th>
                            <th>Kel_Brg</th>
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="card">
                      <!-- /.card-header -->
                      <div class="card-body">
                        <table id="stInvKoreksiSo_x" class="table table-bordered table-striped responsive">
                          <thead>
                          <tr>
                            <th>Kd_Brg</th>
                            <th>Kel_Brg</th>
    ...
    <div class="modal fade" id="modal-delete" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('trDetailKoreksiSo.del') }}" method="post">
                  {{ csrf_field() }}
                  <div class="modal-body">
...
$("#trDetailKoreksiSo").DataTable({
  "responsive": true, 
  "lengthChange": false, 
  "autoWidth": false,
  "order": [],
  "buttons": ["copy", "csv", "excel", "pdf", "print", "colvis"]
}).buttons().container().appendTo('#trDetailKoreksiSo_wrapper .col-md-6:eq(0)');
...
//     $.ajax({
//         type: 'GET',
//         url: "{{URL::to('trDetailKoreksiSo/showinv')}}"
//     }).done( function( response ) {
//         $('#showForm').html(response.html);
//     });
// });

$('#stInvKoreksiSo').DataTable({
  responsive: true,
  processing: true,
  serverSide: true,
  ajax: '{!! route('stInvKoreksiSo.data') !!}', // memanggil route yang menampilkan data json
  columns: 
  [
...
$('#stInvKoreksiSo_x').DataTable({
  responsive: true,
  processing: true,
  serverSide: true,
  ajax: '{!! route('stInvKoreksiSox.data') !!}', // memanggil route yang menampilkan data json
  columns: 
  [
    { // mengambil & menampilkan kolom sesuai tabel database
        data: 'kd_brg',
...
$(document).on('click', '.editDetKoreksiSo', function() {
    let id = $(this).attr('data-id');
    let id_head_koreksi_so = $(this).attr('data-id_head_koreksi_so');
    ...
    let tgl_koreksi = $(this).attr('data-tgl_koreksi');
    // alert(tgl_koreksi);
    
    $('#idx').val(id);
    $('#id_head_koreksi_sox').val(id_head_koreksi_so);
    $('#kd_brgx').val(kdbrg);
    $('#kel_brgx').val(kelbrg);
    $('#part_numbx').val(partnumb);
    $('#ukuranx').val(uk);
    ...
    $('#tglx').val(tgl_koreksi);
    // $("#jns_kayu_m").val(ky).trigger('change');
            
});

$(document).on('click', '.delDetKoreksiSo', function() {
    let id = $(this).attr('data-id');
    $('#id-destroy').val(id);
    $('#id-destroy2').html(id);
});
...
//     $.ajax({
//         type: 'GET',
//         url: "{{ URL::to('trDetailKoreksiSo/showedit')}}"+"/"+id
//     }).done( function( response ) {
